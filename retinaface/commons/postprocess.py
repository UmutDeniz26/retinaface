...
def rotate_landmarks(
    landmarks: dict, angle: float, direction: int, size: Tuple[int, int, int]
) -> dict:
    """
    Rotate the landmarks around the center of the image.

    Args:
        landmarks (dict): Dictionary containing the facial landmarks.
        angle (float): Angle of rotation in degrees.
        direction (int): Direction of rotation (-1 for clockwise, 1 for counterclockwise).
        size (tuple of int): Tuple representing the size of the image (width, height).

    Returns:
        rotated_landmarks (dict): Dictionary containing the new facial landmarks.
    """
    # Angle in radians
    angle = angle * np.pi / 180
    ...
    for key, value in landmarks.items():
        # Translate the landmark to the center of the image
        x = value[0] - size[1] / 2
        y = value[1] - size[0] / 2
        ...
        # Translate the landmark back to the original position
        x_new = x_new + size[1] / 2
        y_new = y_new + size[0] / 2

        rotated_landmarks[key] = (x_new, y_new)
